...
export default function ReporteFuentesReclutamientoPage() {
  ...
  }, [appliedClientId, appliedVacancyId, appliedDateFrom, appliedDateTo])
  ...
  const pieByCandidates = useMemo(
    () =>
      rows.map((r, i) => ({
        name: formatRecruitmentSourceLabel(r),
  ...
  const hiresBarData = useMemo(
    () =>
      [...rows]
        .map((r) => ({
          name: formatRecruitmentSourceLabel(r).slice(0, 16),
          fullName: formatRecruitmentSourceLabel(r),
  ...
  const kpiItems = [
    ...
    },
    {
      ...
      helper: "Promedio simple del % informado por fila",
    },
  ] as const
    ...
    <div className="min-w-0 flex flex-col gap-6 pb-10">
      <section className="px-4 pt-6 md:px-8" aria-label="Encabezado del reporte">
        <PortalPageHeader
          ...
          description="Volumen por canal, contrataciones y conversión. El desglose por etapa depende de campos opcionales del API."
        />
      </section>
      <section className="space-y-4 px-4 md:px-8" aria-label="Filtros y tabla del reporte">
        ...
        {!loading && !error && pieByCandidates.length > 0 ? (
          <div className="grid gap-4 lg:grid-cols-2">
            <ReportesChartCard
              ...
              description="Participación por fuente según candidatos en el periodo."
            ...
            <ReportesChartCard
              title="Contrataciones por fuente"
              description="Qué canal aporta más cierres, no solo postulaciones."
        ...
        <ReportesDataTable<RecruitmentSourceRow>
          ...
          getRowKey={(r, i) =>
            String(
              `${r.sourceKey ?? r.applicationSource ?? "src"}-${i}`
            )
          }
        />
      </section>
