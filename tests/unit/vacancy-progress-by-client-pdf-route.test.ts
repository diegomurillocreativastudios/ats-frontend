...
vi.mock("next/headers", () => ({
  cookies: vi.fn(async () => ({
    get: (name: string) =>
      name === "ats_access_token" ? { value: "test-token-abcdefgh" } : undefined,
  })),
}))
...
describe("POST /api/recruiter/reportes/vacancy-progress-by-client/pdf", () => {
  ...
  it("returns application/pdf with pdfkit-v2 headers for authoritative rows", async () => {
