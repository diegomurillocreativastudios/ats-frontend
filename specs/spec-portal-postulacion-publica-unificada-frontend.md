...
- request `multipart/form-data` (el adjunto de CV, campo `cvFile`, **solo** puede ser `.pdf`),
...
- `cvFile` requerido,
- validar extensión permitida: **únicamente** `.pdf` (no se procesan otros formatos).
...
- campos editables,
- helper text en el upload indicando que solo se acepta PDF.
...
- [ ] El frontend envía `multipart/form-data` al nuevo endpoint público; el CV solo acepta archivos PDF.
- [ ] El frontend muestra `Te has postulado a la vacante exitosamente` al recibir `200`.
- [ ] El frontend maneja correctamente `400`, `404`, `409` y `415`.
