 ...
 * Descarga por id opaco (FE-SEC-020); no usa storagePath en el cliente.
 */
export default function DocumentsList({
  ...
  const ownerId = candidateId.trim()
  ...
  const handleDownload = async (doc: CandidateDocument) => {
    ...
    try {
      const url = resolveBffUrl(
        `/api/candidate/${encodeURIComponent(ownerId)}/documents/${encodeURIComponent(doc.id)}`
      )
      ...
      a.download =
        fromHeader || resolveDocumentDisplayName(doc) || "documento"
    ...
    <div className="flex flex-col gap-3 md:gap-4">
      ...
      {documents.length === 0 ? (
        ...
        <ul className="flex flex-col gap-3" aria-label={t("listAria")}>
          {documents.map((doc) => {
            const displayName = resolveDocumentDisplayName(doc)
            return (
              <li
                key={doc.id}
              ...
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-ats-arena/70 md:h-12 md:w-12">
                  <FileText
                    className="h-5 w-5 text-vo-purple md:h-6 md:w-6"
                    aria-hidden
                  />
                </div>
                ...
                <div className="flex shrink-0 items-center gap-1">
                  <button
                    ...
                    disabled={!ownerId || downloadingId === doc.id}
              ...
              ? t.rich("deleteMessage", {
                  name: () => (
                    <span className="font-medium text-foreground">
                      &ldquo;
                      {resolveDocumentDisplayName(documentPendingDelete)}
                      &rdquo;
                    </span>
                  ),
                })
